...
It first truncates the file to zero length if it exists, creates a file if it does not exist. 

6   a+  Opens a text file for both reading and writing.  
It creates the file if it does not exist.  Start from beginning but writing can only be appended. 
